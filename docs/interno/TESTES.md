...
- [ ] Rodar `migration-074-desconectar-integracao-nome-formulario.sql` antes de testar.
- [ ] **Atenção**: `supabase functions deploy integrations` — sem o deploy, nada disso funciona (a validação do link e o "Desconectar" dependem só do backend).
- [ ] **Bug real corrigido**: o botão "Conectar integração" continuava aparecendo mesmo com uma conexão já conectada. Agora, assim que uma conexão de um Ativo Digital fica "Conectada", o botão vira "Desconectar integração" (vermelho) — clicar reseta a conexão pra "Desconectado" (mantém o histórico da linha, apaga o token OAuth) e o botão "Conectar integração" volta a aparecer. Testar em Ativos Digitais, qualquer ativo já conectado (Google Ads, Meta Ads ou Google Forms).
- [ ] **Bug real corrigido**: conectar um Google Forms colando o link PÚBLICO de resposta (`.../forms/d/e/<id>/viewform`) em vez do link de EDIÇÃO (`.../forms/d/<id>/edit`) terminava "Conectado" mesmo sem funcionar de verdade (o id extraído desse link é só a letra solta "e"). Agora dá erro na hora, antes de nem começar o OAuth: "Esse é o link PÚBLICO de resposta... Cole o link de EDIÇÃO". Testar: colar um link `.../viewform` no diálogo "Conectar integração" → confirma o erro imediato, sem redirecionar pro Google.
- [ ] Colar um link de edição válido, mas de um formulário que essa conta do Google não tem acesso (ou um id que não existe) → completa o login do Google normalmente, mas depois do callback confirma que a conexão fica "Erro" (não "Conectado") com uma mensagem clara pedindo o link de edição certo — antes disso ficava "Conectado" mesmo sem funcionar.
- [ ] **Nome real do formulário + link automático**: conectar um Google Forms com um link de edição válido → confirma que (1) o card do Ativo Digital mostra uma linha "Formulário: <título real>" abaixo do badge Conectado, diferente do nome do Ativo Digital (que é só o rótulo digitado ao cadastrar); (2) o campo "Link de acesso" do Ativo Digital é preenchido sozinho com o link público do formulário (conferir no diálogo de editar o Ativo, ou no link clicável do card); (3) em Ativos Digitais → Integrações, a linha da conexão também mostra "Formulário: <título real>"; (4) abrir "Ver respostas" dessa conexão mostra o título real no cabeçalho do diálogo.
- [ ] Reconectar (desconectar e conectar de novo) o mesmo Ativo com um formulário DIFERENTE → confirma que o nome do formulário e o link de acesso atualizam pro novo, sem misturar com o antigo.
...
- [ ] Rodar `migration-075-arquivamento-automatico-tarefas.sql` antes de testar. Sem deploy de Edge Function — é tudo RPC/frontend.
- [ ] **Não dá pra testar o "30 dias" esperando 30 dias de verdade** — pra confirmar que o arquivamento funciona, force manualmente no SQL Editor: pegue o id de uma tarefa `status = 'done'` (Kanban ou Tarefas do Cliente) e rode `update public.tasks set updated_at = now() - interval '31 days' where id = '<id>';` (ou `update public.client_tasks set completed_at = now() - interval '31 days' where id = '<id>';`, só se essa linha NÃO tiver recorrência) — depois recarregue a página Kanban ou "Tarefas do Cliente".
- [ ] Ao recarregar, confirma: (1) a tarefa forçada some da lista/coluna "Concluída" normal; (2) aparece um toast avisando quantas foram arquivadas; (3) o botão "Arquivadas" no topo da página mostra a contagem entre parênteses.
- [ ] Abrir o diálogo "Arquivadas" → confirma que a tarefa aparece lá com nome do cliente e data "Arquivada em".
- [ ] Clicar no ícone de restaurar → confirma que a tarefa volta a aparecer na lista normal (Kanban ou Tarefas do Cliente) e some da lista de arquivadas.
- [ ] Forçar de novo, abrir "Arquivadas", clicar no ícone de apagar → confirma a tela de confirmação, e que depois de confirmar a tarefa some de vez (checar que ela realmente não existe mais no banco).
